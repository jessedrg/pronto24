...
async function getStats(dateRange?: string) {
  ...
  let dateFilter = ""
  let dateFilterWithWhere = ""
  let intervalFilter = "7 days"

  switch (dateRange) {
    case "today":
      dateFilter = "AND DATE(created_at) = CURRENT_DATE"
      dateFilterWithWhere = "WHERE DATE(created_at) = CURRENT_DATE"
      intervalFilter = "1 day"
      break
    case "yesterday":
      dateFilter = "AND DATE(created_at) = CURRENT_DATE - 1"
      dateFilterWithWhere = "WHERE DATE(created_at) = CURRENT_DATE - 1"
      intervalFilter = "2 days"
      break
    case "week":
      dateFilter = "AND created_at >= NOW() - INTERVAL '7 days'"
      dateFilterWithWhere = "WHERE created_at >= NOW() - INTERVAL '7 days'"
      intervalFilter = "7 days"
      break
    case "month":
      dateFilter = "AND created_at >= NOW() - INTERVAL '30 days'"
      dateFilterWithWhere = "WHERE created_at >= NOW() - INTERVAL '30 days'"
      intervalFilter = "30 days"
      break
    case "all":
    default:
      dateFilter = ""
      dateFilterWithWhere = ""
      intervalFilter = "365 days"
  }
  ...
  const [
    ...
    safeQuery(
      sqlClient`SELECT COUNT(*) as count FROM leads ${dateFilterWithWhere ? sqlClient.unsafe(dateFilterWithWhere) : sqlClient``}`,
      [{ count: 0 }],
    ),
    safeQuery(
      sqlClient`SELECT COUNT(*) as count, COALESCE(SUM(lead_price), 0) as revenue FROM leads WHERE status = 'accepted' ${dateFilter ? sqlClient.unsafe(dateFilter) : sqlClient``}`,
      [{ count: 0, revenue: 0 }],
    ),
    safeQuery(sqlClient`SELECT COUNT(*) as count FROM leads WHERE DATE(created_at) = CURRENT_DATE`, [{ count: 0 }]),
    safeQuery(
      sqlClient`SELECT COUNT(*) as count, COALESCE(SUM(lead_price), 0) as revenue FROM leads WHERE status = 'accepted' AND DATE(created_at) = CURRENT_DATE`,
      [{ count: 0, revenue: 0 }],
    ),
    safeQuery(
      sqlClient`SELECT COUNT(*) as count, COALESCE(SUM(lead_price), 0) as revenue FROM leads WHERE status = 'pending' AND DATE(created_at) = CURRENT_DATE`,
      [{ count: 0, revenue: 0 }],
    ),
    safeQuery(sqlClient`SELECT COUNT(*) as count FROM leads WHERE created_at >= NOW() - INTERVAL '7 days'`, [
      { count: 0 },
    ]),
    safeQuery(
      sqlClient`SELECT COUNT(*) as count, COALESCE(SUM(lead_price), 0) as revenue FROM leads WHERE status = 'accepted' AND created_at >= NOW() - INTERVAL '7 days'`,
      [{ count: 0, revenue: 0 }],
    ),
    safeQuery(
      sqlClient`SELECT COUNT(*) as count, COALESCE(SUM(lead_price), 0) as revenue FROM leads WHERE status = 'pending' AND created_at >= NOW() - INTERVAL '7 days'`,
      [{ count: 0, revenue: 0 }],
    ),
    safeQuery(
    ...
    safeQuery(
      sqlClient`SELECT service, COUNT(*) as total, SUM(CASE WHEN status = 'accepted' THEN 1 ELSE 0 END) as sold, COALESCE(SUM(CASE WHEN status = 'accepted' THEN lead_price ELSE 0 END), 0) as revenue, COALESCE(SUM(CASE WHEN status = 'pending' THEN lead_price ELSE 0 END), 0) as potential FROM leads ${dateFilterWithWhere ? sqlClient.unsafe(dateFilterWithWhere) : sqlClient``} GROUP BY service`,
      [],
    ),
    safeQuery(sqlClient`SELECT COUNT(*) as count FROM partners WHERE active = true`, [{ count: 0 }]),
  ])
  ...
  const partnersList = await safeQuery(
    sqlClient`
      ...
      FROM partners p
      LEFT JOIN leads l ON l.partner_id = p.id AND l.status = 'accepted'
  ...
  const funnelStats = await safeQuery(
    ...
    WHERE created_at >= NOW() - INTERVAL '${sqlClient.unsafe(intervalFilter)}'
